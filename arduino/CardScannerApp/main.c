...
int main(void)
{
	...
	Serial.println("Initialization complete.  Entering main routine.");
...
void readCard()
{
	...
	LCD.setCursorPosition(LCD_LINE_4_START);
	LCD.write( STRING_SCAN );
	...
	LCD.setCursorPosition(LCD_LINE_4_START);
	LCD.write( STRING_PROCESS );
	LCD.setCursorPosition(LCD_LINE_4_START);
	LCD.enableBlink(true);
	delay(1500);
	LCD.enableBlink(false);

	LCD.setCursorPosition(LCD_LINE_4_START);
	randNum = random(100);
	if( randNum > 80)
	{
		LCD.write( STRING_DENIED);
	}
	else
	{
		LCD.write( STRING_GRANTED);
	}

	delay(1500);
	LCD.setCursorPosition(LCD_LINE_4_START);
	LCD.write( STRING_BUTTON);
...
void initialize()
{
	...
	// Interrupt once a second based on
	// rising edge from RTC.
	attachInterrupt( 0, update, RISING );
}

/**
 * Updates the sample flag
 */
void update()
{
	sampleReady = true;
