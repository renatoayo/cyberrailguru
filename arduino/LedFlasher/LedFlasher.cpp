...
#define MODE_LAST					MODE_ALTERNATE_1


//TODO will likely need an IFDEF here for HW specific things
uint8_t input_pin[NUM_INPUTS] = {SW1, SW2, POT1, POT2, POT3,   POT4, POT5, POT6 };
uint8_t input_value[NUM_INPUTS];
uint8_t output_pin[NUM_OUTPUTS] = { OUT1, OUT2, OUT3, OUT4, OUT5, OUT6 };
uint8_t output_value[NUM_OUTPUTS];

uint8_t map_value[NUM_INPUTS];
...
void loop()
{
	...
	for(i=0; i<NUM_INPUTS; i++)
	{
		input_value[i] = analogRead(input_pin[i]);
	}
	...
	for(i=0; i<NUM_INPUTS; i++)
	{
		Serial.print("INPUT");
		Serial.print(i, DEC);
		Serial.print( input_value[i], DEC );
		Serial.print(" ");
	}
	Serial.println("");
#endif
	...
	case MODE_ALTERNATE_1: // slow alternating, 600ms
		//( startPin,  endPin,  flash,  onTime,  offTime,  breakTime,  fadeTime)
		map_value[0] = map(POT1, IN_LOW, IN_HIGH, 100, 1000); // on time
		map_value[1] = map(POT2, IN_LOW, IN_HIGH, 0, 1000);
		sequence(0, 2, 1, map_value[0], 0, map_value[1], 0);
		break;
	...
	case MODE_ALTERNATE_2: // double flash, alternating, 60ms on, 30ms off, 200ms break
		map_value[0] = map(POT1, IN_LOW, IN_HIGH, 10, 500); // on time
		map_value[1] = map(POT2, IN_LOW, IN_HIGH, 0, 1000);
		//( startPin,  endPin,  flash,  onTime,  offTime,  breakTime,  fadeTime)
		sequence(0, 2, 2, map_value[0], 30, map_value[1], 0);
		break;
...
void writeOutput(uint8_t pinIndex, uint8_t outputValue)
{
	if( outputValue > MAX_VALUE)
	{
		outputValue = MAX_VALUE;
	}
	analogWrite(output_pin[pinIndex], outputValue);
}
